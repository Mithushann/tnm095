...
class SnakeNN:
    def __init__(self, initial_games = 1200, test_games = 125, goal_steps = 2000, lr = 1e-2, filename = 'snake_nn_2.tflearn'):
    ...
    def initial_population(self):
        ...
        for _ in range(self.initial_games):
            ...
            for _ in range(self.goal_steps):
                ...
                done, score, snake, food,_  = game.play_step(game_action)
                if done:
                    self.n_games +=1 
    ...
    def train_model(self, training_data, model):
        #plot(training_data)
        #plot(training_data, self.filename)
    ...
    def visualise_game(self, model):
        ...
        for _ in range(self.goal_steps):
            ...
            game_action = self.get_game_action(snake, action - 1)
            done, _, snake, food,_  = game.play_step(game_action)
            if done:
                break
            else:
                prev_observation = self.generate_observation(snake, food)
