...
class SnakeNN:
    ...
    def initial_population(self): #skapar tränings data
        training_data = []
        for _ in range(self.initial_games):
            game = SnakeGameAI() # replaced with our env
            ...
            for _ in range(self.goal_steps):
                ...
                done,_, snake,_,_  = game.play_step(game_action)
                if done:
    ...
    def generate_action(self, snake):
        action = randint(0,2) - 1 #[-1, 0, 1] => [left, straight , right ]
         # Action
        # [1,0,0] -> Straight
        # [0,1,0] -> Right Turn 
        # [0,0,1] -> Left Turn
    ...
    def is_direction_blocked(self, snake, direction):
        point = np.array(snake[0]) + np.array(direction) # new head position
        return point.tolist() in snake[1:] or point[0] == 0 or point[1] == 0 or point[0] == 620 or point[1] == 460
