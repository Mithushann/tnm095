...
def mapChrom2Weights(chrom, bitsPerWeight, numInputs, numHiddenLayerNodes, numOutputs):
	"""
    Function to initialize the structure of the list of weights. Translate the chromosome 
    bit string into a list structure.
	...
	Returns:
		...
		It always contains 3 lists: 
        - the first for weights between the input layer and hidden layer 1
		- the second for the weights between hidden layer 1 and hidden layer 2,
		- the third for the weights between hidden layer 2 and the output layer.
	...
	#While we are still looking at the bits representing weights going into the output layer
...
def calcHiddenLayerOutputs(inputList, weightList, numHiddenLayerNodes):
	...
	This function is always called from testNetwork().
...
def calcOutputForNeuron(inputList, weights):
	...
	It is called from within testNetwork().
	
    Arguments:
		inputList: A list of all inputs going into the specific node of the neural network.
		weights: A list of all the weights going into the specific node.
	Returns:
