from agent_GA import Snake
import model_GA as mGA
import population_model_GA as pGA 
import os

class SnakeGameGATrain():
	"""
	Attributes:
	...
	def __init__(self, fps, population, chroms_per_gen, bits_per_weight, num_inputs, num_hidden_layer_nodes, num_outputs):
        ...
        Arguments:
			...
			bits_per_weight: The number of bits per each weight in the neural network.
		...
		self.weights = mGA.mapChrom2Weights(self.population[self.cur_chrom], self.bits_per_weight, self.num_inputs, self.num_hidden_layer_nodes, self.num_outputs)
		self.fitness_scores = []
		self.game_scores = []
	...
	def game_over(self):
		...
		if self.cur_chrom == self.chroms_per_gen:
			...
			next_generation, best_individual, best_fitness, average_fitness = pGA.createNextGeneration(self.population, self.fitness_scores)
